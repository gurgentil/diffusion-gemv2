function[obsObjects] = getObstructingObjectsV2X(commPairs,objectCells,...
    objectMidpoints,BigBoxes,numRowsPerObject,logicalOrObjects,verbose,...
...
%   objectMidpoints:    midpoints of objects
...
if iscell(objectMidpoints)
    % Contains both vehicles and RSUs
    RSUMidpoints = objectMidpoints{1};
    vehicleMidpoints = objectMidpoints{2};
else 
    % Vehicles only
    vehicleMidpoints = objectMidpoints;
end

if size(commPairs,2) == 2
    if iscell(objectMidpoints)
        % These are pointers to the RSU and vehicle midpoints
        commPairBoxes(:,[2, 1]) = RSUMidpoints(commPairs(:,1),:);
...
elseif size(commPairs,2) == 4
    % This is experimental data (Tx-Rx coordinates: x1,y1,x2,y2)
    commPairBoxes = commPairs;
...
if size(commPairs,2) == 4
    % In case of experiments, switch positio of rows 1&3 with 2&4
    temp1 = commPairBoxesTemp(:,1);
    temp2 = commPairBoxesTemp(:,3);
    commPairBoxesTemp(:,1) = commPairBoxesTemp(:,2);
    commPairBoxesTemp(:,3) = commPairBoxesTemp(:,4);
    commPairBoxesTemp(:,2) = temp1;
    commPairBoxesTemp(:,4) = temp2;
end
            ...
            if bigbox(1,7)~=-1
                ...
                if size(commPairs,2) == 2
                    if iscell(objectMidpoints)
                        ...
                        x1 = RSUMidpoints(commPairs(currIndices,1),1);
                        y1 = RSUMidpoints(commPairs(currIndices,1),2);
                ...
                elseif size(commPairs,2) == 4 || size(commPairs,2) == 6 ...
                    || size(commPairs,2) == 5
                    % Comm. paira are either experimental data or
                    % coordinates of vehicle-wallPoint pairs for
                ...
                if numel(obsPairsIndices)>0
                    if logicalOrObjects
                        ...
                        if numberOfArguments == 10
                            ...
                            if size(commPairList,2)==3 && ...
                                    commPairList(1,3)==-Inf
                                % In case of experiments, commPairList is
                                % used to contain Tx and Rx height.
                                TxHeights = repmat(commPairList(1),...
                                    length(obsPairsIndices),1);
                                RxHeights = repmat(commPairList(2),...
                                    length(obsPairsIndices),1);
                            else
                                % Otherwise, it's a list of comm. pair IDs.
